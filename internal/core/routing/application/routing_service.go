...
import (
	...
	"go_hex/internal/core/routing/ports/primary"
...
func (s *RoutingApplicationService) FindOptimalItineraries(ctx context.Context, routeSpec primary.RouteSpecification) ([]primary.Itinerary, error) {
	// Check permissions
...
func (s *RoutingApplicationService) convertToExternalFormat(candidates []routeCandidate) []primary.Itinerary {
	var itineraries []primary.Itinerary

	for _, candidate := range candidates {
		var legs []primary.Leg

		for _, leg := range candidate.legs {
			externalLeg := primary.Leg{
